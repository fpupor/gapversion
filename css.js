	...
	CSS = new Class({
		...
		getRule: function (selector) {
			...
			$foreach(embeds.rules, function (rules, x) {
				$foreach(rules, function (rule, i) {
					if (!rule.style || !such.hasSelector('^' + selector.toLowerCase() + '$', rule))
						return;
		...
		parseSelectors: function(selector){
			selector = (selector) ? selector.replace(/^\w+/, function (m) {
				return m.toLowerCase();
			}) : '';

			return String(selector).toLowerCase();
		},
		
		hasSelector: function(selector, rule){
			var selectorText = such.parseSelectors(rule.selectorText);
			
			var regTest = new RegExp(selector);

			return (selectorText && regTest.test(selectorText));
		},
