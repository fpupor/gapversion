...
DOM = new Class({
	...
	CUSTONSELECTOR	:{
		...
		'parent': function(instance, value){
			instance.result = instance.result.concat(instance.parent(instance.scope));
			return false;
		},
		'parents': function(instance, value){
			instance.result = instance.result.concat(instance.parents(instance.scope, value));
			return false;
		}
	},
	...
	parents: function(element, selector){
		var parent = element.tagName != 'BODY' ? element['parentNode'] : null;
		var result = [];
		
		if(parent){
			if(selector)
				result = DOM.get(selector, parent);
			else
				result.push(parent);
				
			return result.concat(such.parents(parent, selector));
		}
		
		return result;
	},
	
	fristChild: function(element){
